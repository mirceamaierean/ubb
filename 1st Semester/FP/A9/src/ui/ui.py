...
class UI:
  ...
  def run(self):
    ...
    option = int(input())
    if option == 1:
      print("You selected to play with AI. \n Select a difficulty: \n 1. Easy \n 2. Medium \n 3. Hard")
...
class GUI:
  def __init__(self):
    ...
    self.__screen = pygame.display.set_mode((WIDTH * self.SQUARESIZE, (HEIGHT + 1) * self.SQUARESIZE))
    self.__font = pygame.font.SysFont("comic sans", 65)
    self.__images = [pygame.transform.scale(pygame.image.load("src/static/images/red.png"), (90, 90)), pygame.transform.scale(pygame.image.load("src/static/images/yellow.png"), (90, 90))]
  ...
  def draw_board(self):
    ...
    for col in range(WIDTH):
      for row in range(HEIGHT + 1):
        pygame.draw.rect(self.__screen, self.BLUE, (col * self.SQUARESIZE, row * self.SQUARESIZE + self.SQUARESIZE, self.SQUARESIZE, self.SQUARESIZE))
        if row > 0:
          pygame.draw.circle(self.__screen, self.FILL_COLOR, (int(col * self.SQUARESIZE + (self.SQUARESIZE >> 1)), int(row * self.SQUARESIZE + (self.SQUARESIZE >> 1))), self.RADIUS)
    
    for col in range(WIDTH):
      for row in range(HEIGHT):
        if self.__game.board.board[row][col] >= 1:
          self.__screen.blit(self.__images[self.__game.board.board[row][col] - 1], self.get_appropriate_coordinates(HEIGHT - row, col))
    
    pygame.display.update()

  def display_message(self, message: str, position: int):
      pygame.draw.rect(self.__screen, self.FILL_COLOR, (0, 0, WIDTH * self.SQUARESIZE, self.SQUARESIZE))
  ...
  def draw_button(self, message: str, rect_start_height: int, label_width: int, label_height: int):
      ...
      pygame.draw.rect(self.__screen, self.FILL_COLOR, (270, rect_start_height, 180, 35))
      pygame.draw.rect(self.__screen, self.BUTTON, (273, rect_start_height + 2, 174, 31))
  ...
  def draw_piece(self, col):
      ...
      if self.__game.winning_move(col):
          ...
          self.display_message("Player " + str((self.__game.turn ^ 1) + 1) + " wins!", 150)
      if self.__game.is_draw():
          self.__game.game_over = True
  ...
  def play_with_ai(self, depth: int):
    ...
    while not self.__game.game_over:
      ...
      if self.__game.turn == 1 and not self.__game.game_over:
        col = self.__ai.minimax(self.__game.board, self.__depth, -math.inf, math.inf, True)[0]
        self.draw_piece(col)
