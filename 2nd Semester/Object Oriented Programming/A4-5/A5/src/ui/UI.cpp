...
void UI::printAllMovies()
{
  ...
  std::cout << "Number of movies: " << movies.getSizeOfDynamicVector() << "\n";
  for (int index = 0; index < movies.getSizeOfDynamicVector(); ++index)
    std::cout << index << ". " << movies[index].getTitle() << " " << movies[index].getGenre() << " " << movies[index].getTrailer() << " " << movies[index].getYearOfRelease() << " " << movies[index].getLikesCount() << "\n";
}
...
void UI::userGetMoviesByGenre()
{
  ...
  DynamicVector<int> vectorOfIndexesOfMovies = this->service.getPositionsOfMoviesByGenre(genreOfMovieThatTheUserIsSearchingAfterInOrderToAddToHisWishlist);
  if (vectorOfIndexesOfMovies.getSizeOfDynamicVector() == 0)
  {
    std::cout << "There are no movies with the given genre!\n";
    return;
  }
  int currentIndexOfDynamicVector = 0;
  while (vectorOfIndexesOfMovies.getSizeOfDynamicVector())
  {
    Movie movieThatIsCurrentlyDisplayes = this->service.getMovieByPosition(vectorOfIndexesOfMovies[currentIndexOfDynamicVector]);
    ...
    this->service.openTrailerOfMovieInBrowser(vectorOfIndexesOfMovies[currentIndexOfDynamicVector]);
    ...
    if (choiceOfUser == 'y')
    {
      this->service.addIndexOfMovieToUserListByPosition(vectorOfIndexesOfMovies[currentIndexOfDynamicVector]);
      std::cout << "Movie added to your wishlist!\n";
      vectorOfIndexesOfMovies.removeElementFromDynamicVector(currentIndexOfDynamicVector);
      --currentIndexOfDynamicVector;
    }
    else
    ...
    {
      ++currentIndexOfDynamicVector;
      if (currentIndexOfDynamicVector >= vectorOfIndexesOfMovies.getSizeOfDynamicVector())
        currentIndexOfDynamicVector = 0;
    }
    else
      break;
  }
...
{
  DynamicVector<int> vectorOfIndexesOfMovies = this->service.getIndexesOfMoviesThatTheUserLikes();
  if (vectorOfIndexesOfMovies.getSizeOfDynamicVector() == 0)
  {
    std::cout << "There are no movies in your wishlist!\n";
    return;
  }
  for (int index = 0; index < vectorOfIndexesOfMovies.getSizeOfDynamicVector(); ++index)
  {
    Movie currentMovieToBeDisplayed = this->service.getMovieByPosition(vectorOfIndexesOfMovies[index]);
    std::cout << index << ". " << currentMovieToBeDisplayed.getTitle() << " " << currentMovieToBeDisplayed.getGenre() << " " << currentMovieToBeDisplayed.getYearOfRelease() << " " << currentMovieToBeDisplayed.getLikesCount() << "\n";
  }
}
