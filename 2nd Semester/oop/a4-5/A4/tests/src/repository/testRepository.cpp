...
void TestsRepository::testRepository()
{
  ...
  Movie movieToAdd("The Shawshank Redemption", "Drama", "https://www.youtube.com/watch?v=6hB3S9bIaco", 1994, 1000000);
  genericRepository.addMovie(movieToAdd);
  assert(genericRepository.getMoviesCount() == 1);
  // Test removing a movie from the repository
  Movie movieToRemove("The Shawshank Redemption", "Drama", "https://www.youtube.com/watch?v=6hB3S9bIaco", 1994, 1000000);
  genericRepository.removeMovie(movieToRemove);
  assert(genericRepository.getMoviesCount() == 0);
  // Test updating a movie from the repository
  Movie movieToUpdate("The Shawshank Redemption", "Drama", "https://www.youtube.com/watch?v=6hB3S9bIaco", 1994, 1000000);
  genericRepository.addMovie(movieToUpdate);

  Movie updatedMovie("IT", "Horror", "https://www.youtube.com/watch?v=FnCdOQsX5kc", 2017, 1000000);
  ...
  assert(genericRepository.getMovieByPosition(0).getLikesCount() == 1000000);
  ...
  assert(genericRepository.getMovies().getElement(0).getLikesCount() == 1000000);
