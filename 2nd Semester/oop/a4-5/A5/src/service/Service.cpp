...
DynamicVector<int> Service::getPositionsOfMoviesByGenre(const std::string &genreOfMovie)
{
  DynamicVector<int> positionsOfMoviesByGenre;

  for (int index = 0; index < this->repository.getMoviesCount(); ++index)
  {
    std::string genreOfCurrentMovie = this->repository.getMovieByPosition(index).getGenre();
...
DynamicVector<int> Service::getIndexesOfMoviesThatTheUserLikes() const
{
...
{
  this->indexesOfMoviesThatTheUserLikes.addElementInDynamicVector(positionOfMovieThatTheUserHasJustLiked);
}
...
void Service::removeIndexOfMovieFromUserList(int indexOfMovieThatTheUserHasJustDisliked)
{
  this->indexesOfMoviesThatTheUserLikes.removeElementFromDynamicVector(indexOfMovieThatTheUserHasJustDisliked);
}