...
impl SymbolTable {
    fn new(capactity: Option<usize>) -> SymbolTable {
    ...
    fn put(&mut self, item: (String, usize)) {
        ...
        loop {
            ...
            if Option::is_some(&self.hash[hash_pos]) {
                ...
                println!("Collision detected for {} with {}", item.0, current_item.0);
            }
...
fn main() {
    println!("Hello, world! This will be the assignment containing the hashset");
    ...
    println!("{:?}", st.get("hello".to_string()));
    println!("{:?}", st.get("world".to_string()));
    println!("{:?}", st.get("this".to_string()));
    println!("{:?}", st.get("is".to_string()));
    println!("{:?}", st.get("a".to_string()));
    println!("{:?}", st.get("test".to_string()));
    println!("{:?}", st.get("not".to_string()));
