...
mod tests {
    ...
    fn test_insert_existing_variable() {
        ...
        assert_eq!(st.get("hello"), Some(0));
        // assert that world is part of the symbol table
        assert_eq!(st.get("world"), Some(1));
        assert_eq!(st.get("this"), Some(2));
        assert_eq!(st.get("is"), Some(3));
        assert_eq!(st.get("test"), Some(4));
    }
